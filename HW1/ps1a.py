...
def greedy_cow_transport(cows,limit=10):
    ...
    # TODO: Your code here
    trips = []
    cowsorted = sorted(cows.items(), key=lambda x:int(x[1]), reverse=True)
    def findtrip(cowsorted, limit):
        trip = []
        for cow in cowsorted:
            if int(cow[1]) <= limit:
                ...
                trip.append(cow[0])
                print(cow[0])
                cowsorted.remove(cow)
        return trip, cowsorted
    while len(cowsorted) > 0:
        trip, cowsorted = findtrip(cowsorted, limit)
        trips.append(trip)
    return trips
        ...
        if len(partition) > minum:
            ...
            for trip in partition:
                ...
                if totalweight > limit:
                    y = 1
                    break
            if y == 0 and len(partition) < minum:
                minum = len(partition)
